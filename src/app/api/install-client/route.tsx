...
export async function GET()
{
    ...
    const binaryUrl = new URL(`${protocol}://${forwardedHost}/bin/tightvnc-setup-64bit.msi`);
    ...
    const SHOW_WALPAPER = [ `SET_REMOVE_WALLPAPER=1`, `VALUE_OF_REMOVEWALLPAPER=0` ];
    const HIDE_ICON = [ `SET_RUNCONTROLINTERFACE=1`, `VALUE_OF_RUNCONTROLINTERFACE=0` ];

    const ENFORCE_PASSWORD_FOR_SETTINGS_CONTROL = [ `SET_USECONTROLAUTHENTICATION=1`, `VALUE_OF_USECONTROLAUTHENTICATION=1` ];
    const REQUIRE_PASSSWORD_FOR_CONNECTIONS = [ `SET_USEVNCAUTHENTICATION=1`, `VALUE_OF_USEVNCAUTHENTICATION=1` ];
    const MASTER_CONTROL_PASSWORD = [ `SET_CONTROLPASSWORD=1`, `VALUE_OF_CONTROLPASSWORD="${masterPassword}"` ];
    const CONNECTION_PASSWORD = [ `SET_PASSWORD=1`, `VALUE_OF_PASSWORD="${vncPassword}"` ];

    const installationCommandLine = ([ `/quiet`, `/norestart`, `ADDLOCAL="Server"` ].concat(
        ALWAYS_SHARED,
        SHOW_WALPAPER,
        HIDE_ICON,
