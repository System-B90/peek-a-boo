...
export const ClassesProvider = ({
    ...
    const rawHiveClassToClass = useCallback(
        (rawClass: RawHiveClass): HiveClass => {
            const classType = rawClass.type;
            assert(
                ["Room", "Student Group", "Level"].includes(classType),
                `Class type ${classType} is not recognized!`,
            );
            return {
                id: rawClass.id,
                name: rawClass.name,
                ...
                users: rawClass.users.map(
                    (userId) => getStudentInfoByHiveId(userId)?.studentUsername,
                ),
    ...
    useEffect(() => {
        queryHiveClasses()
            .then((rawClasses) => {
                setClasses(
                    rawClasses.map((rawClass) => rawHiveClassToClass(rawClass)),
                );
            })
            .catch((error) => {
...
export function useClasses() {
    ...
    if (context.default) {
        throw Error("useClasses must be used inside ClassesProvider!");
    }
    return context;
}
