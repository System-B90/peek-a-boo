...
export const QueryParamsProvider = ({
    ...
    const parseCommaSeparated = useCallback(
    ...
    const buildQueryParams = useCallback(
        (newFilters: Array<string>, newActives: Array<string>) => {
    ...
    useEffect(() => {
        ...
        if (desired === currentSearchString) {
            return;
        }
    ...
    useEffect(() => {
        ...
        const newFilters = parseCommaSeparated(
            searchParams.get(FILTER_QUERY_PARAM),
        );
        const newActives = parseCommaSeparated(
            searchParams.get(ACTIVE_QUERY_PARAM),
        );

        /* eslint-disable react-hooks/set-state-in-effect -- syncing React state from the URL (an external system) on navigation */
        setFilters(
            (prev) =>
                filterParamExists
                    ? arraysEqual(prev, newFilters)
                        ? prev
                        : newFilters
...
export function useQueryParams() {
    ...
    if (context.default) {
        throw new Error(
            "useQueryParams must be used inside QueryParamsProvider",
        );
    }
    return context;
}
