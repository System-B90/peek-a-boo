...
export const StudentInfoProvider = ({
    ...
    const [currentExerciseParentModuleId, setCurrentExerciseParentModuleId] =
        useState<number>(0);
    const [
        currentExerciseParentModuleParentSubjectId,
        setCurrentExerciseParentModuleParentSubjectId,
    ] = useState<number>(0);
    ...
    useEffect(() => {
        ...
        getStudentInfo(studentUsername)
            .then((data) => {
                if (!data) {
                    return;
                }
                ...
                setCurrentExerciseParentModuleId(
                    data.currentExerciseParentModuleId,
                );
                setCurrentExerciseParentModuleParentSubjectId(
                    data.currentExerciseParentModuleParentSubjectId,
                );
                setCurrentExerciseUrl(
                    `https://${clientEnvConfig.HIVE_HOSTNAME}/course/${data.currentExerciseParentModuleParentSubjectId}/${data.currentExerciseParentModuleId}/${data.currentExerciseId}#${data.hiveId}`,
                );
