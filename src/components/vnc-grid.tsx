...
import { VncCard, VncCardSkeleton } from "@/components/vnc-card";

export function VncGrid() {
    const { activeStudents } = useActiveStudents();
    const { removeActive, initialized: queryParamsInitialized } =
        useQueryParams();

    const onClose = useCallback(
        (username: string) => removeActive(username),
        [removeActive],
    );

    const vncCards = queryParamsInitialized
        ? [...activeStudents].map((username) => (
            <VncCard
                key={`vnc-card-${username}`}
                onClose={onClose}
                studentUsername={username}
            />
        ))
        : [...Array(3).keys()].map((i) => (
            <VncCardSkeleton key={`vnc-card-skeleton-${i}`} />
        ));
